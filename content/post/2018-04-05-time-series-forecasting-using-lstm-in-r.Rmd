...
The gates takes in, as input, the hidden states from previous time step $ h_{t-1} $ and the current input $ x_{t} $ and multiply them pointwise by weight matrices, $ W $, and bias $ b $ is added to the product.
...
## Normalize the data
Just like in any other neural network models, we rescale the input data X to the range of the activation function. As shown earlier the default activation function is sigmoid whose range is [-1, 1]. The code below will help transform our data to the range [-1, 1]
...
We set the argument *stateful*= TRUE, 
