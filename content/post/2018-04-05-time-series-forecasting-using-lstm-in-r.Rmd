...
***
Time series involves data collected sequentially in time. I denote univariate data by $$ y_{t} \in \mathbb{R} $$  where `$$ t \in \mathcal{T} $$` is the time indexing when the data was observed. The time ``t`` can be discrete in which case `$$ \mathcal{T} = \mathbb{Z}$$` or continuous  where now `$$ \mathcal{T} = \mathbb{R}$$`. For simplicity of the analysis we will consider only discrete time series.
