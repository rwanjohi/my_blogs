...
* A section with the header **Results**.  In this section there are two columns that floats next to each other, with headers **Predicted Object** and **Likelihood**. This is  where names of predicted images and their respective likelihood will be displayed upon prediction.
...
*  The jQuery **change** function triggers change event after the Image is selected from the browse button: 

      1.  Load the file object via the FileReader
      2.  Read the file from the file object and log the base64 encoded content to the console. 
      3.  Clears the texts in the **Results** sections when a new image is selected. 
